...
let differenceGraphQL = users_GraphQlArr.filter(
  (x) => !users_RESTArr.includes(x)
).length;
let differenceREST = users_RESTArr.filter(
  (x) => !users_GraphQlArr.includes(x)
).length;
...
console.log(
  "Остаток REST: ",
  (differenceREST / (users_RESTArr.length + users_GraphQlArr.length)) * 100
);
console.log(
  "Остаток GraphQL: ",
  (differenceGraphQL / (users_RESTArr.length + users_GraphQlArr.length)) * 100
);
console.log(
  "Пересечение (коньюнкция): ",
  (dif.size / (users_RESTArr.length + users_GraphQlArr.length)) * 100 * 2
);
